...
extension AttributedString.Guts {
    ...
    func _finalizeTrackedIndicesUpdate(mutationStartOffset: Int, utf8LengthDelta: Int) {
        ...
        for idx in 0 ..< trackedRanges.count {
            ...
            if lowerBound.utf8Offset > mutationStartOffset, utf8LengthDelta != 0 {
            ...
            if upperBound.utf8Offset > mutationStartOffset, utf8LengthDelta != 0 {
...
extension AttributedString {
    /// Tracks the location of the provided index throughout the mutation closure, returning a new, updated index that represents the same effective location after the mutation
    /// - Parameters:
    ...
    /// - Returns: the updated `Index` that is valid after the mutation has been performed, or `nil` if the mutation performed does not allow for tracking to succeed (such as replacing the provided inout variable with an entirely different AttributedString)
    public mutating func transform<E>(updating index: Index, mutation: (inout AttributedString) throws(E) -> Void) throws(E) -> Index? {
        try self.transform(updating: Range(uncheckedBounds: (index, index)), mutation: mutation)?.lowerBound
    }
    ...
    public mutating func transform<E>(updating ranges: [Range<Index>], mutation: (inout AttributedString) throws(E) -> Void) throws(E) -> [Range<Index>]? {
        ...
        _guts.trackedRanges.append(contentsOf: ranges.map(\._bstringRange))
