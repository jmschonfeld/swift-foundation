...
extension AttributedString.Guts {
    ...
    func _prepareStringMutation(
        in range: Range<BigString.Index>
    ) -> (mutationStartUTF8Offset: Int, oldUTF8Count: Int, invalidationRange: Range<Int>) {
        ...
        return (utf8TargetRange.lowerBound, self.string.utf8.count, invalidationRange)
    }

    func _finalizeStringMutation(
        _ state: (mutationStartUTF8Offset: Int, oldUTF8Count: Int, invalidationRange: Range<Int>)
    ) {
        let utf8Delta = self.string.utf8.count - state.oldUTF8Count
        self._finalizeTrackedIndicesUpdate(mutationStartOffset: state.mutationStartUTF8Offset, utf8LengthDelta: utf8Delta)
